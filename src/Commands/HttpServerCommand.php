...
class HttpServerCommand extends Command
{
    ...
    protected function getHotReloadProcess($server)
    {
        ...
        $options = Arr::get($this->config, 'hot_reload.fswatch_options');
        $eventTypes = Arr::get($this->config, 'hot_reload.fswatch_event_type');
        $log = Arr::get($this->config, 'hot_reload.log');

        $cb = function (array $events) use ($server, $log) {
            if ($log) {
                foreach($events as $event) {
                    $this->info(FSOutput::format($event));
                }
            }
            $server->reload();
        };

        return (new FSProcess($options, $recursively, $directory, $eventTypes))->make($cb);
    }
